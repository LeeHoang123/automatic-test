    ...
    stages {
        ...
        stage('Checkout from Git') {
            steps {
                git branch: 'main', url: 'https://github.com/LeeHoang123/automatic-test.git'
                script {
                    // Kiểm tra sự tồn tại của thư mục database và file Person.sql
                    if (!fileExists('database/Person.sql')) {
                        error "File Person.sql không tồn tại trong thư mục database!"
                    }
                }
            }
}
